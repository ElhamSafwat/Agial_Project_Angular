...
export class CalendarTeacherComponent implements OnInit {
  ...
  getEventsForCurrentMonth() {
    ...
    this.user_id = this.myservce.getUserId();

    this.cal_service
      .get_teacher_data(this.user_id, this.month, this.year)
      .subscribe({
        next: (data: any) => {
          ...
          this.events = data.map((event: CustomCalendarEvent) => {
            ...
            return {
              ...
              title: `${event.class_name} - ${event.period}`,
              color: eventColor,
              meta: {
                room: event.room,
                start: event.start,
                ...
                period: event.period,
                class_name: event.class_name, // إضافة class_name إلى الـ meta
              },
            } as CalendarEvent;
          });
